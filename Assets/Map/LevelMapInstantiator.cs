...
namespace Map
{
    public class LevelMapInstantiator : Singleton<LevelMapInstantiator>
    {
        ...
        _playButton.onClick.AddListener(()=>
        {
            ...
            Debug.Log("Hence the level progress is " + PlayerPrefsManager.LevelProgress);
            //SetPosition();
        ...
        {
            Debug.Log("Hence the level progress is " + PlayerPrefsManager.LevelProgress);
            int level = PlayerPrefs.GetInt(PlayerPrefsManager.LevelProgress, 1); // Default to 1 if not found
            float contentY = GetComponent<RectTransform>().anchoredPosition.y;
            float targetY = level * 700f;
        ...
        private void SetLevelImages()
        {
            ...
            for (int i = 0; i < imagesNoToInstantiate; i++)
            {
                ...
                if (level == i + startLevel)
                {
                    ...
                    {
                        image.transform.GetChild(1).GetComponent<RectTransform>().localScale = Vector3.one ;
                    }
                }
            }
        }
        ...
        private IEnumerator NextLevelAnimation(Transform oldLevel, Transform newLevel)
        {
            ...
            if (oldLevel) oldLevel.GetChild(1).GetComponent<RectTransform>().localScale = Vector3.one;
            newLevel.GetChild(1).GetComponent<RectTransform>().localScale = Vector3.one;
            ...
            while (elapsedTime < 1f)
            {
                ...
                if (oldLevel)
                    oldLevel.GetChild(1).GetComponent<RectTransform>().localScale =
                        Vector3.Lerp(Vector3.one, Vector3.one, elapsedTime);
